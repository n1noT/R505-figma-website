...
export default function Footer() {
    ...
    // gérer la regeneration infin du bandeau
    useEffect(() => {
        const followerElement = document.querySelector('#follower');
        if (followerElement) {
            followerElement.classList.add('hidden');
            setTimeout(() => {
    ...
    return (
        <footer className={`flex justify-center items-left md:items-center p-8 md:p-12 w-full ${colors[randomColor]} flex-col gap-4 relative overflow-hidden`}>
            <div className="flex flex-col md:flex-row justify-center items-left gap-8 ">
                ...
                <ul className="flex gap-8 flex-col md:flex-row">
                    <li>
                        <p className="text-4xl lg:w-min font-bold text-wrap">
                    ...
                    <li className="flex flex-row gap-8">
                        ...
                        <p className="text-wrap max-w-64 md:max-w-80">This site is powered by  Harper + Scott, a Certified B-Corporation. For more information read Harper + Scott's CSR policy.</p>
            ...
            <ul className="flex gap-4 flex-col text-lg md:flex-row md:text-3xl mb-16">
                <li>
                    <Link href={"/"} className="hover:underline">
                        FIGMA
                    </Link>
            ...
            <p className="absolute bottom-0 text-4xl animate-bandeau uppercase font-bold text-nowrap">Figma's marvelously considered collection of objects for our time on</p>
            <p className="absolute bottom-0 text-4xl animate-bandeau uppercase font-bold text-nowrap" id="follower">Figma's marvelously considered collection of objects for our time on</p>
        </footer>
    )
